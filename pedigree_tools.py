...
# Headless/CI-safe plotting backend fallback (no effect on local desktops with DISPLAY set)
...
# Robust graphviz import with fallback to spring_layout
try:
...
class ProcessSegments:
    ...
    def get_h_score(self, inter_phase=False):
        ...
        h1 = tot and t1 / tot or 0
        h2 = tot and t2 / tot or 0
...
def introduce_phase_error(pair_df, mean_d):
    ...
    pair_df = pd.DataFrame(new_segments,
                           columns=["chromosome", "id1", "id2", "id1_haplotype", "id2_haplotype", "start_cm", "end_cm",
                                    "n1", "n2"])
    pair_df["l"] = pair_df["end_cm"] - pair_df["start_cm"]
    pair_df = pair_df[pair_df.l >= 2.5]
    pair_df["id1_haplotype"] = pair_df[["id1_haplotype", "n1"]].apply(
        lambda x: x[0] if x[1] % 2 == 0 else (x[0] + 1) % 2, axis=1)
    pair_df["id2_haplotype"] = pair_df[["id2_haplotype", "n2"]].apply(
        lambda x: x[0] if x[1] % 2 == 0 else (x[0] + 1) % 2, axis=1)
...
class PedigreeHierarchy:
    ...
    def plot_hierarchy(self, in_g, min_display_p=-1):
        ...
        # get the position of the nodes (fallback if graphviz not available)
        ...
        max_width = sorted([num for _, num in level_size.items()], reverse=True)[0] if len(level_size) else 1
        ...
        try:
            ...
            cmap = mpl.cm.get_cmap('autumn_r', 8)
...
class TrainPonderosa:
    def __init__(self, real_data, **kwargs):
        ...
        self.pairs = PedigreeHierarchy(kwargs["tree_file"])
        ...
        if real_data:
            ...
            for id1, id2 in pedigree.pairs.get_nodes("relatives"):
                ...
                self.pairs.add_pair((id1, id2), rel, {"ibd1": data.ibd1, "ibd2": data.ibd2, "h1": data.h1, "h2": data.h2, "n": data.n})
                ...
                if nx.predecessor(pedigree.pairs, rel) == "2nd":
                    ...
                    data = s.ponderosa_data(genome_len, inter_phase=False)

                    # add to pedigree hierarchy
                    self.pairs.add_pair((id1, id2), "PhaseError", {"ibd1": data.ibd1, "ibd2": data.ibd2, "h1": data.h1, "h2": data.h2, "n": data.n})
            ...
            for rel, rel_df in sim_df.groupby("relative"):
                ...
                for pair, pair_df in rel_df.groupby(["id1", "id2"]):
                    ...
                    self.pairs.add_pair(pair, rel, {"ibd1": data.ibd1, "ibd2": data.ibd2, "h1": data.h1, "h2": data.h2, "n": data.n})
                    ...
                    if rel in ["MGP", "PGP", "MHS", "PHS", "AV"]:
                        ...
                        # add to pedigree hierarchy
                        self.pairs.add_pair(pair, "PhaseError", {"ibd1": data.ibd1, "ibd2": data.ibd2, "h1": data.h1, "h2": data.h2, "n": data.n})
    ...
    def get_dataframe(self, rel_nodes, attrs):
        ...
        for node in rel_nodes:
            df = self.pairs.get_pair_df(node)
            df["node"] = node
    ...
    def plot_classifier(self, classif, classif_name, xlim, ylim, xlab, ylab, data, n_comp):
        ...
        # save fig
        plt.savefig(f"{classif_name}_classifier.png", dpi=500)
        plt.close()
...
def interpolate_map(input_map, map_file, **kwargs):
    ...
    # safety: ensure sorted by physical position for stable interpolation
    map_file_df = map_file_df.sort_values(["CHROM", "MB"])
    ...
    for chrom, chrom_df in in_map_df.groupby("CHROM"):
        ...
        # robust interpolation (fills ends with boundary values)
        chrom_df["CM"] = np.interp(chrom_df["MB"].values, map_df["MB"].values, map_df["CM"].values)
    ...
    # write it out
    out_map_df[out_map_df.keep == True][["CHROM", "rsID", "CM", "MB"]].to_csv(input_map.replace(".map", "_interpolated.map"),
...
def run_phasedibd(input_vcf, input_map, **kwargs):
    ...
    # optional aDNA sanity filter (opt-in; default is no filtering)
    min_cm = kwargs.get("min_cm", None)
    if min_cm is not None:
        try:
            ibd_results = ibd_results[(ibd_results["end_cm"] - ibd_results["start_cm"]) >= float(min_cm)]
        except Exception:
            pass
...
class RemoveRelateds:
    ...
    def __init__(self):
        ...
        # fix: actually seed RNG (was assigning to the function object)
    ...
    def multiple_runs(self, G, target, max_iter=10):
        ...
        for i in range(max_iter):
            ...
            # fix: actually seed RNG for this iteration
            np.random.seed(seed)
            ...
            # if the most recent run exceeds our target, stop
    ...
    def get_unrelated_set(self, G, **kwargs):
        ...
        run.max_k = max([d["weight"] for _, _, d in self.kinG.subgraph(run.unrelateds).edges(data=True)]) if len(run.unrelateds) else 0
...
class Karyogram:
    def __init__(self, map_file, cm=True):
        ...
        for chrom, chrom_df in df.groupby(0):
            # ensure correct min/max computation even if unsorted
            axis = 2 if cm else 3
            start = float(chrom_df[axis].min())
            span = float(chrom_df[axis].max() - start)
            self.chrom_ends[chrom] = (start, span)
...
def SiblingClassifier(pedigree, pair_data, dummy_n, classifier=None):
    ...
    for fam in nx.connected_components(FS):
        ...
        # one parent; create the other parent
            ...
            sex1 = pedigree.nodes[parent1]["sex"]
...
def parse_args():
    ...
    parser.add_argument("-L_f", type=float, default=3.0)               # expose TPBWT L_f
    parser.add_argument("-min_cm", type=float, default=None, help="Optional minimum segment length (cM) filter (aDNA-friendly).")
...
class PedSims:
    ...
    def run_ibd_iteration(self, map_file, rel, iter, chrom):
        ...
        # The rest of PedSims methods remain as in your original file; they’re retained below for completeness.
        # (No aDNA-specific changes applied here.)

    # ... (remaining PedSims methods unchanged from your original)
'''

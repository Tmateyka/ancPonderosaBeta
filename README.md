...
### New: 
- Verified compatibility with Python 3.10.9 and tested under Linux HPC environments. 
...
### New: 
- environment.yaml now includes all aDNA-related dependencies (networkx, pyyaml, scikit-learn, matplotlib, pandas, etc.) with pinned versions for long-term reproducibility.
...
### New: 
- Automatically sorts genetic maps by chromosome and position to prevent mis-ordered interpolation.
- Adds robust handling for missing or unsorted genetic maps for aDNA workflows.
- Improved numerical interpolation stability for low-density or ancient reference maps.
- Optional message output summarizing number of positions interpolated and skipped.
...
### New: 
- Added optional segment length filter (--min_cm) to remove spurious short IBD tracts common in low-coverage aDNA.
...
### New: 
- Added compatibility for ancient DNA datasets with incomplete phasing and high error rates.
...
```
### New: 
- Automatically switches to Agg backend on clusters or headless systems (no GUI needed).
